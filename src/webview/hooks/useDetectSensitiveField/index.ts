...
import type { EnvironmentVariable } from '@bruno-types';

const VARIABLE_NAME_REGEX = /\{\{([^}]+)\}\}/g;
const ENV_VAR_REFERENCE_REGEX = /^\s*\{\{.*\}\}\s*$/;

interface SensitiveFieldResult {
  showWarning: boolean;
  warningMessage?: string;
}

interface UseDetectSensitiveFieldReturn {
  isSensitive: (value: string | null | undefined) => SensitiveFieldResult;
}

export const useDetectSensitiveField = (collection: AppCollection | null | undefined): UseDetectSensitiveFieldReturn => {
  ...
  const isEnvVarReference = (value: string): boolean => {
  ...
  const extractVarNames = (value: string | null | undefined): string[] => {
  ...
  const isVarNotSecret = (varName: string, envVars: EnvironmentVariable[] = []): boolean => {
    const found = envVars.find((v) => v.name === varName);
    return found !== undefined && !found.secret;
  };

  const isSensitive = (value: string | null | undefined): SensitiveFieldResult => {
