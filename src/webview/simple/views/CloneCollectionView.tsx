...
import { browseCloneLocation, cloneCollection } from '../ipc-actions';
...
interface CloneCollectionViewProps {
  collection: { name: string; pathname: string } | null;
}
...
const CloneCollectionView: React.FC<CloneCollectionViewProps> = ({ collection }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      setIsCloning(true);
      try {
        await cloneCollection(values.name, values.name, values.location, collection.pathname);
  ...
  const handleBrowse = async () => {
    try {
      const result = await browseCloneLocation();
      if (result) {
