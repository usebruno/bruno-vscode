...
export class MaskedEditor {
  ...
  maskChar: any;
  maskTimeout: any;
  constructor(editor: any, maskChar = '*') {
    this.editor = editor;
  ...
  handleBeforeChange(cm: any, changeObj: any) {
    if (!this.enabled || this.isProcessing) return;
   ...
   */
  applyCharacterMasking(content: string) {
    let currentLine = 0;
    let currentCh = 0;
   ...
   */
  applyLineMasking(lineCount: number) {
 ...
 */
export function createMaskedEditor(editor: unknown, maskChar?: string): MaskedEditor {
 ...
 */
export function supportsMasking(editor: any): unknown {
  return editor
