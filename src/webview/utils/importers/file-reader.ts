...
const readFile = (file: File) => {
  return new Promise<{ fileName: string; content: unknown }>((resolve, reject) => {
    const fileReader = new FileReader();
    fileReader.onload = (e) => {
      try {
        const result = e.target?.result;
        if (typeof result !== 'string') {
          reject(new BrunoError(`Unable to read file: ${file.name}`));
          return;
        }
...
export const readMultipleFiles = async (files: any) => {
