...
export const updateUidsInCollection = (_collection: any) => {
  const collection = cloneDeep(_collection);

  collection.uid = uuid();

  const updateItemUids = (items: any[] = []) => {
    each(items, (item) => {
      item.uid = uuid();
...
export const transformItemsInCollection = (collection: any) => {
    ...
    each(items, (item) => {
      if (['http', 'graphql', 'grpc', 'ws'].includes(item.type)) {
        ...
        if (item.request.query) {
          item.request.params = item.request.query.map((queryItem: any) => ({
            ...queryItem,
            type: 'query',
        ...
        each(get(item, 'examples'), (example) => {
          if (['http', 'graphql', 'grpc', 'ws'].includes(example.type)) {
            ...
            if (example.request && example.request.query) {
              example.request.params = example.request.query.map((queryItem: any) => ({
                ...queryItem,
                type: 'query',
                uid: queryItem.uid || uuid()
              }));
...
export const hydrateSeqInCollection = (collection: any) => {
  const hydrateSeq = (items: any[] = []) => {
    let index = 1;
    each(items, (item) => {
