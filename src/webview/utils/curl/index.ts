...
export const getRequestFromCurlCommand = (curlCommand: any, requestType = 'http-request') => {
  const parseFormData = (parsedBody: any) => {
    const formData: any = [];
  ...
  try {
    ...
    const body: {
      ...
      formUrlEncoded: unknown[] | null;
      graphql: unknown | null;
      file: string | null;
    } = {
      mode: 'none',
      json: null,
