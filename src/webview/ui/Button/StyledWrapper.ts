import styled, { css, keyframes, FlattenInterpolation, ThemeProps, DefaultTheme } from 'styled-components';
import { rgba, darken } from 'polished';

type SizeKey = 'xs' | 'sm' | 'base' | 'md' | 'lg';
type RoundedKey = 'sm' | 'base' | 'md' | 'lg' | 'full';
type FontWeightKey = 'regular' | 'medium';
...
const sizeStyles: Record<SizeKey, FlattenInterpolation<ThemeProps<DefaultTheme>>> = {
  xs: css`
...
const roundedStyles: Record<RoundedKey, FlattenInterpolation<ThemeProps<DefaultTheme>>> = {
  sm: css`
...
const fontWeightStyles: Record<FontWeightKey, number> = {
  regular: 400,
  medium: 500
};

// For secondary, use text color for outline/ghost; for others, use bg
const getDisplayColor = (colorConfig: any, colorName: any) => {
  return colorName === 'secondary' ? colorConfig.text : colorConfig.bg;
};

const getVariantStyles = (variant: any, color: any) => {
  if (variant === 'filled') {
    return css`
...
type VariantKey = 'filled' | 'outline' | 'ghost';
...
const StyledWrapper = styled.div<ButtonWrapperProps>`
  ...
  button {
    ...
    width: ${(props) => (props.$fullWidth ? '100%' : 'auto')};
    font-family: inherit;
    font-weight: ${(props) => fontWeightStyles[props.$fontWeight as FontWeightKey] || 400};
    line-height: 1;
    cursor: pointer;
    ...
    ${(props) => sizeStyles[props.$size as SizeKey]}
    ${(props) => roundedStyles[props.$rounded as RoundedKey]}
