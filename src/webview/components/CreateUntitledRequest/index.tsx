...
interface CreateUntitledRequestProps {
  collectionUid?: string;
  itemUid?: string;
  onRequestCreated?: (...args: unknown[]) => void;
  placement?: React.ReactNode;
}


const CreateUntitledRequest = ({
  collectionUid,
  itemUid = null,
  onRequestCreated,
  placement = 'bottom'
  ...
  const collection = collections?.find((c: any) => c.uid === collectionUid);
  ...
  const handleCreateHttpRequest = useCallback(async () => {
    ...
    (dispatch(
      ...
      .catch((err: any) => toast.error(err ? err.message : 'An error occurred while adding the request'));
