...
const EnvironmentDetails = ({
  ...
  const environments = isGlobal ? globalEnvironments : (collection?.environments || []);
  ...
  const validateEnvironmentName = (name: any) => {
    ...
    const isDuplicate = (environments || []).some(
      (env: any) => env?.uid !== environment.uid && env?.name?.toLowerCase().trim() === trimmedName
    );
    if (isDuplicate) {
  ...
  const handleSaveRename = () => {
    ...
    const action = isGlobal
      ? renameGlobalEnvironment({ name: newName, environmentUid: environment.uid })
      : renameEnvironment(newName, environment.uid, collection.uid);
  ...
  return (
    <StyledWrapper>
      {openDeleteModal && (
        <DeleteEnvironment onClose={() => setOpenDeleteModal(false)} environment={environment} collection={collection} isGlobal={isGlobal} />
      )}
      {openCopyModal && (
        <CopyEnvironment onClose={() => setOpenCopyModal(false)} environment={environment} collection={collection} isGlobal={isGlobal} />
      )}
      ...
      <div className="content">
        <EnvironmentVariables environment={environment} setIsModified={setIsModified} collection={collection} isGlobal={isGlobal} />
      </div>
    </StyledWrapper>
  );
};
