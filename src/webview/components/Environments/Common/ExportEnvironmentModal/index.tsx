...
const ExportEnvironmentModal = ({
  ...
  const truncateEnvName = (name: any) => {
    if (name.length > 40) {
  ...
  const browse = () => {
    (dispatch(browseDirectory()) as unknown as Promise<string | null>)
      .then((dirPath: any) => {
  ...
  const handleEnvironmentToggle = (envUid: any) => {
  ...
  const handleSelectAll = () => {
    const allSelected = environments.every((env: any) => selectedEnvironments[env.uid]) || false;

    const newSelection = environments.reduce((acc: Record<string, boolean>, env: { uid: string }) => ({
      ...acc,
      [env.uid]: !allSelected
    }), {}) || {};
  ...
  const selectedEnvs = useMemo(() => {
    return environments.filter((env: any) => selectedEnvironments[env.uid]) || [];
              ...
              {environments && environments.length > 0 ? (
                <div className="flex flex-col h-full">
                  <div className="flex justify-between items-center mb-2 pb-1">
                    ...
                    >
                      {environments.every((env: any) => selectedEnvironments[env.uid]) ? 'Deselect All' : 'Select All'}
                    </button>
                  </div>
                  <div className="flex flex-col gap-1 flex-1 overflow-y-auto">
                    {environments.map((env: any) => <label key={env.uid} className="environment-item">
                      <input
                        type="checkbox"
