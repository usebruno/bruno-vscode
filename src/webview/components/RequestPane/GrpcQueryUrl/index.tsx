...
interface STREAMING_METHOD_TYPESProps {
...
const GrpcQueryUrl = ({
  ...
  const isConnectionActive = useSelector((state: any) => state.collections.activeConnections.includes(item.uid));
  ...
  const onMethodDropdownCreate = (ref: any) => methodDropdownRef.current = ref;
  const onProtoDropdownCreate = (ref: any) => protoDropdownRef.current = ref;
  ...
  const handleReflection = async (url: any, isManualRefresh = false) => {
    const { methods, error } = await reflectionManagement.loadMethodsFromReflection(url, isManualRefresh);

    if (error) {
      toast.error(`Failed to load gRPC methods: ${(error as Error).message || 'Unknown error'}`);
      return;
    }
    ...
    if (methods && methods.length > 0) {
      const haveSelectedMethod = selectedGrpcMethod && methods.some((method: any) => method.path === selectedGrpcMethod.path);
      if (!haveSelectedMethod) {
        setSelectedGrpcMethod(null);
        onMethodSelect({ path: '', type: '' });
      } else if (selectedGrpcMethod) {
        const currentMethod = methods.find((method: any) => method.path === selectedGrpcMethod.path);
        if (currentMethod) {
  ...
  const handleProtoFileLoad = async (filePath: any, isManualRefresh = false) => {
  ...
  const handleGrpcurl = async (url: any) => {
    if (!url) {
    ...
    try {
      const result = await (dispatch(generateGrpcurlCommand(item, collection.uid)) as unknown as Promise<{ success: boolean; command?: string; error?: string }>);
  ...
  const handleGrpcMethodSelect = (method: any) => {
  ...
  const handleEndConnection = (e: any) => {
    e.stopPropagation();
  ...
  const handleReflectionModeToggle = (e: React.MouseEvent) => {
    e.stopPropagation();
    e.preventDefault();
    ...
    <StyledWrapper className="flex items-center relative" data-testid="grpc-query-url-container">
      ...
      <div className="flex items-center w-full input-container h-full relative">
        <SingleLineEditor
          ...
          onSave={onSave}
          theme={storedTheme}
          onChange={(newValue: string) => debouncedOnUrlChange(newValue)}
          onRun={handleRun}
