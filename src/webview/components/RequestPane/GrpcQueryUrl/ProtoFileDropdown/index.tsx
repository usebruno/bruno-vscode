...
interface ProtoFileDropdownProps {
  collection: any;
  item: any;
  isReflectionMode?: boolean;
  protoFilePath?: string;
  showProtoDropdown?: boolean;
  setShowProtoDropdown: (value: boolean | ((prev: boolean) => boolean)) => void;
  onProtoDropdownCreate?: (ref: any) => void;
  onReflectionModeToggle?: (e: React.MouseEvent) => void;
  onProtoFileLoad?: (path: string) => void;
}
...
const ProtoFileDropdown = ({
  ...
  onProtoFileLoad
}: ProtoFileDropdownProps) => {
  const { theme } = useTheme();
  const dispatch = useDispatch();
  const [activeTab, setActiveTab] = useState('protofiles'); // 'protofiles' or 'importpaths'
  const protoFileManagement = useProtoFileManagement(collection);
  const invalidProtoFiles = protoFileManagement.protoFiles.filter((file: any) => !file.exists);
  const invalidImportPaths = protoFileManagement.importPaths.filter((path: any) => !path.exists);

  const handleSelectProtoFile = async (e: React.MouseEvent) => {
    e.stopPropagation();
    const { success, filePath, error } = await protoFileManagement.browseForProtoFile();
    if (!success) {
      if (error) {
        toast.error(`Failed to browse for proto file: ${(error as Error).message || 'Unknown error'}`);
      }
      return;
    }
    ...
    if (!addSuccess) {
      if (addError) {
        toast.error(`Failed to add proto file: ${(addError as Error).message || 'Unknown error'}`);
      }
      return;
    }
  ...
  const handleSelectCollectionProtoFile = (protoFile: any) => {
  ...
  const handleBrowseImportPath = async (e: React.MouseEvent) => {
    e.stopPropagation();
    const { success, directoryPath, error } = await protoFileManagement.browseForImportDirectory();
    if (!success) {
      if (error) {
        toast.error(`Failed to browse for import directory: ${(error as Error).message || 'Unknown error'}`);
      }
      return;
    }
    ...
    if (!addSuccess) {
      if (addError) {
        toast.error(`Failed to add import path: ${(addError as Error).message || 'Unknown error'}`);
      }
      return;
    }
  ...
  const handleToggleImportPath = async (index: number) => {
    const { success, enabled, error } = await protoFileManagement.toggleImportPathFromRequest(index);
    if (!success) {
      if (error) {
        toast.error(`Failed to toggle import path: ${(error as Error).message || 'Unknown error'}`);
      }
      return;
    }
  ...
  const handleOpenCollectionProtobufSettings = (e: React.MouseEvent) => {
    e.stopPropagation();
    dispatch(openCollectionSettings(collection.uid, 'protobuf'));
  };

  const ProtoFileDropdownIcon = forwardRef<HTMLDivElement>((props, ref) => {
    return (
      <div ref={ref} className="proto-file-dropdown-container" onClick={() => setShowProtoDropdown((prev: any) => !prev)} data-testid="grpc-proto-file-dropdown-icon">
        {!isReflectionMode && (
