...
interface GraphQLSchemaActionsProps {
  item: unknown;
  collection: unknown;
  onSchemaLoad?: (...args: unknown[]) => void;
  toggleDocs?: React.ReactNode;
}


const GraphQLSchemaActions = ({
  item,
  collection,
  onSchemaLoad,
  toggleDocs
  ...
  const schemaDropdownTippyRef = useRef<{ hide: () => void } | null>(null);
  const onSchemaDropdownCreate = (ref: any) => schemaDropdownTippyRef.current = ref;

  const MenuIcon = forwardRef<HTMLDivElement>((props, ref) => {
    return (
