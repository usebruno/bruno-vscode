...
interface DEFAULT_SETTINGSProps {
...
const Settings = ({
  ...
  const getPropertyFromDraftOrRequest = (propertyKey: any) => item.draft ? get(item, `draft.${propertyKey}`, {}) : get(item, propertyKey, {});
  ...
  const updateSetting = useCallback((settingUpdate: any) => {
  ...
  const onMaxRedirectsChange = useCallback((e: any) => {
  ...
  const onTimeoutChange = useCallback((e: any) => {
    const value = e.target.value;
    if (value === '' || /^\d+$/.test(value)) {
      const numericValue = value === '' ? 0 : parseInt(value, 10);
  ...
  const handleTimeoutDropdownSelect = useCallback((option: any) => {
    if (option === 'inherit') {
      ...
      <div className="bruno-form">
        ...
        <div className="flex flex-col gap-4">
          ...
          <InheritableSettingsInput
            ...
            onValueChange={(e: any) => !isTimeoutInherited && onTimeoutChange(e)}
