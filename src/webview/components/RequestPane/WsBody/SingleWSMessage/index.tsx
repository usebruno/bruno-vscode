...
interface SingleWSMessageProps {
  message: {
    name?: string;
    content?: string;
    type?: string;
  ...
  methodType?: string;
  handleRun?: () => void;
  canClientSendMultipleMessages?: boolean;
  isLast?: boolean;
}


export const TYPE_BY_DECODER: Record<string, string> = {
  base64: 'binary',
  json: 'json',
  xml: 'xml'
};

export const DECODER_BY_TYPE: Record<string, string> = invert(TYPE_BY_DECODER);
...
export const SingleWSMessage = ({
  ...
  const [messageFormat, setMessageFormat] = useState<string>(autoDetectLang(content) as string);

  const onUpdateMessageType = (type: any) => {
  ...
  const codemirrorMode: Record<string, string> = {
