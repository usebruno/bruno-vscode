...
interface MessageToolbarProps {
  index?: number;
  canClientStream?: React.ReactNode;
  isConnectionActive?: boolean;
  onSend?: (...args: unknown[]) => void;
  onRegenerateMessage?: (...args: unknown[]) => void;
  onPrettify?: (...args: unknown[]) => void;
  onDeleteMessage?: (...args: unknown[]) => void;
  showDelete?: boolean;
  message?: React.ReactNode;
  item?: React.ReactNode;
  collection?: React.ReactNode;
  methodType?: React.ReactNode;
  handleRun?: (...args: unknown[]) => void;
  canClientSendMultipleMessages?: React.ReactNode;
  isLast?: boolean;
}
...
const SingleGrpcMessage = ({
  ...
  const [reflectionCache] = useLocalStorage<Record<string, any[]>>('bruno.grpc.reflectionCache', {});
  const [protofileCache] = useLocalStorage<Record<string, any[]>>('bruno.grpc.protofileCache', {});
  ...
  const onRegenerateMessage = async () => {
    try {
      ...
      if (protoPath) {
        ...
        if (cachedMethods) {
          methodMetadata = cachedMethods.find((method: any) => method.path === methodPath);
        }
      } else if (url) {
...
const GrpcBody = ({
  ...
  const messagesToShow = body.grpc.filter((_: any, index: any) => canClientSendMultipleMessages || index === 0);
