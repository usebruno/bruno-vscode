...
const MultipartFormParams = ({
  ...
  const handleParamsChange = useCallback((updatedParams: any) => {
  ...
  const handleBrowseFiles = useCallback((row: any, onChange: any) => {
    (dispatch(browseFiles([], [])) as unknown as Promise<string[]>)
      .then((filePaths: string[]) => {
        ...
        const updatedParams = (currentParams || []).map((p: any) => {
          if (p.uid === row.uid) {
  ...
  const handleClearFile = useCallback((row: any) => {
  ...
  const handleValueChange = useCallback((row: any, newValue: any, onChange: any) => {
    const currentParams = params || [];
    const existingParam = currentParams.find((p: any) => p.uid === row.uid);
    if (existingParam) {
      const updatedParams = currentParams.map((p: any) => {
        if (p.uid === row.uid) {
  ...
  const getFileName = (filePaths: any) => {
          ...
          <div className="flex items-center value-cell">
            <div className="flex-1">
              <MultiLineEditor
                ...
                theme={storedTheme}
                value={value || ''}
                onChange={(newValue: any) => handleValueChange(row, newValue, onChange)}
                onRun={handleRun}
                allowNewlines={true}
