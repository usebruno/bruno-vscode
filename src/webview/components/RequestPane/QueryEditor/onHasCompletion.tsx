 ...
 */
export default function onHasCompletion(_cm: any, data: any, onHintInformationRender: any) {
  const CodeMirror = require('codemirror');

  let information: any;
  let deprecation: any;

  CodeMirror.on(data, 'select', (ctx: any, el: any) => {
    ...
    const description = ctx.description ? DOMPurify.sanitize(md.render(ctx.description)) : 'Self descriptive.';
    ...
    if (ctx && deprecation && ctx.deprecationReason) {
      const reason = ctx.deprecationReason ? DOMPurify.sanitize(md.render(ctx.deprecationReason)) : '';
