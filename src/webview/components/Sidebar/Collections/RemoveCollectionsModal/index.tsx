...
const getDisplayItems = (items: any, maxWidth = MAX_COLLECTIONS_WIDTH) => {
  const visibleItems = [];
  let totalWidth = 0;
...
const RemoveCollectionsModal = ({
  ...
  const allDrafts = useMemo(() => {
    const requestDrafts: any = [];
    const collectionDrafts: any = [];
    const folderDrafts: any = [];

    collectionUids.forEach((collectionUid: any) => {
      ...
      unsavedFolders.forEach((folder: any) => {
        folderDrafts.push({
          name: folder.name,
  ...
  const collectionsWithUnsavedChanges = useMemo((): { uid: string; name: string }[] => {
    ...
    return Object.keys(draftsByCollection)
      ...
      .filter((c): c is { uid: string; name: string } => c !== null);
  ...
  const handleCloseAllCollections = () => {
    const removalPromises = collectionUids.map((uid: any) => dispatch(removeCollection(uid)));
