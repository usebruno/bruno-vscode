...
const Notifications = () => {
  ...
  const { version } = useApp() as { version?: string };
  ...
  const handleNotificationItemClick = (notification: any) => (e: any) => {
    e.preventDefault();
    setSelectedNotification(notification);
    dispatch(markNotificationAsRead({ notificationId: notification?.id }));
  };

  const getSanitizedDescription = (description: any) => {
    ...
    <div className="flex flex-row gap-8">
      ...
      {unreadNotifications.length > 0 && (
        <>
          ...
          <button
            className="select-none text-link mark-as-read cursor-pointer hover:underline"
              ...
              {notifications?.length > 0 ? (
                <div className="grid grid-cols-4 flex flex-row">
                  <div className="col-span-1 flex flex-col">
                    ...
                    >
                      {notifications?.slice(notificationsStartIndex, notificationsEndIndex)?.map((notification: any) => <li
                        key={notification.id}
