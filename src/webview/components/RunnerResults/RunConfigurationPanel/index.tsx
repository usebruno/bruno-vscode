  ...
  useEffect(() => {
    ...
    try {
      ...
      if (savedConfiguration?.requestItemsOrder?.length > 0) {
        ...
        const requestMap = new Map(requests.map((req: any) => [req.uid, req]));

        savedConfiguration.requestItemsOrder.forEach((uid: any) => {
  ...
  const handleRequestSelect = useCallback((item: any) => {
    try {
      if (selectedItems.includes(item.uid)) {
        const newSelectedUids = selectedItems.filter((uid: any) => uid !== item.uid);
