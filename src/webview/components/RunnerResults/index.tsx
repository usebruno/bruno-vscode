...
const getDisplayName = (fullPath: any, pathname: any, name = '') => {
  let relativePath = path.relative(fullPath, pathname);
  const { dir = '' } = path.parse(relativePath);
  return path.join(dir, name);
};

const getTestStatus = (results: any) => {
  if (!results || !results.length) return 'pass';
  const failed = results.filter((result: any) => result.status === 'fail');
  return failed.length ? 'fail' : 'pass';
};

const allTestsPassed = (item: any) => {
...
const FILTERS: Record<string, FilterConfig> = {
  all: {
    label: 'All',
    predicate: () => true,
    resultFilter: (results: any) => results
  },
  passed: {
    label: 'Passed',
    predicate: (item: any) => allTestsPassed(item),
    resultFilter: (results: any) => results?.filter((r: any) => r.status === 'pass')
  },
  failed: {
    label: 'Failed',
    predicate: (item: any) => anyTestFailed(item),
    resultFilter: (results: any) => results?.filter((r: any) => ['fail', 'error'].includes(r.status))
  },
  skipped: {
    label: 'Skipped',
    predicate: (item: any) => item.status === 'skipped',
    resultFilter: (results: any) => results
  }
};
...
export default function RunnerResults({
  ...
  const filterTestResults = (results: any) => {
                  ...
                  <div className="item-path mt-2">
                    ...
                    {tagsEnabled && areTagsAdded && item?.tags?.length > 0 && (
                      <div className="pl-7 text-xs text-muted">
                        Tags: {item.tags.filter((t: any) => tags.include.includes(t)).join(', ')}
                      </div>
                    )}
                    ...
                    <ul className="pl-8">
                      {item.preRequestTestResults
                        ? filterTestResults(item.preRequestTestResults).map((result: any) => <li key={result.uid}>
                        {result.status === 'pass' ? (
                      ...
                      {item.postResponseTestResults
                        ? filterTestResults(item.postResponseTestResults).map((result: any) => <li key={result.uid}>
                        {result.status === 'pass' ? (
                          <span className="test-success flex items-center">
                            <IconCheck size={18} strokeWidth={2} className="mr-2" />
                            {result.description}
                        ...
                        : null}
                      {item.testResults
                        ? filterTestResults(item.testResults).map((result: any) => <li key={result.uid}>
                        {result.status === 'pass' ? (
                          <span className="test-success flex items-center">
                            <IconCheck size={18} strokeWidth={2} className="mr-2" />
                            {result.description}
                        ...
                        : null}
                      {filterTestResults(item.assertionResults).map((result: any) => <li key={result.uid}>
                        {result.status === 'pass' ? (
                          <span className="test-success flex items-center">
                            <IconCheck size={18} strokeWidth={2} className="mr-2" />
