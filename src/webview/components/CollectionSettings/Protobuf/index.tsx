...
const ProtobufSettings = ({
  ...
  const handleRemoveProtoFile = async (index: any) => {
  ...
  const handleBrowseClick = async () => {
    ...
    if (result.success && result.filePath) {
      await addProtoFileToCollection(result.filePath);
    }
  };

  const handleReplaceProtoFile = async (index: any) => {
    const result = await browseForProtoFile();
    if (result.success) {
  ...
  const handleReplaceImportPath = async (index: any) => {
  ...
  const handleRemoveImportPath = async (index: any) => {
    await removeImportPathFromCollection(index);
  };

  const handleToggleImportPath = async (index: any) => {
  ...
  return (
    <StyledWrapper className="h-full w-full">
      <div className="mb-6" data-testid="protobuf-proto-files-section">
        ...
        <div>
          {protoFiles.some((file: any) => !file.exists) && (
          ...
          <table className="w-full border-collapse" data-testid="protobuf-proto-files-table">
            ...
            <tbody>
              {protoFiles.length === 0 ? (
                ...
                protoFiles.map((file: any, index: any) => {
      ...
      <div className="mb-6" data-testid="protobuf-import-paths-section">
        ...
        <div>
          {importPaths.some((path: any) => !path.exists) && (
          ...
          <table className="w-full border-collapse" data-testid="protobuf-import-paths-table">
            ...
            <tbody>
              {importPaths.length === 0 ? (
                ...
                importPaths.map((importPath: any, index: any) => {
