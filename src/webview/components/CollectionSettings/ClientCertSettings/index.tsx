...
const ClientCertSettings = ({
  ...
  const certFilePathInputRef = useRef<HTMLInputElement>(null);
  const keyFilePathInputRef = useRef<HTMLInputElement>(null);
  const pfxFilePathInputRef = useRef<HTMLInputElement>(null);
  ...
  const handleRemove = (indexToRemove: any) => {
    const updatedCerts = clientCertConfig.filter((cert: any, index: any) => index !== indexToRemove);
    const clientCertificates = {
      enabled: true,
      certs: updatedCerts
    };
        ...
        <div className="mb-3 flex items-center">
          ...
          <div className="textbox flex flex-row items-center w-[300px] h-[1.70rem] relative">
            <SingleLineEditor
              ...
              theme={storedTheme}
              onChange={(val: any) => formik.setFieldValue('passphrase', val)}
