...
interface FORMAT_ICONSProps {
  formatOptions: unknown[];
  formatValue?: React.ReactNode;
  onFormatChange?: (...args: unknown[]) => void;
  onPreviewTabSelect?: (...args: unknown[]) => void;
  selectedTab?: unknown;
  isActiveTab?: boolean;
...
const FORMAT_ICONS: Record<string, typeof IconBraces> = {
  json: IconBraces,
  html: IconCode,
  xml: IconFileCode,
...
const ButtonIcon = forwardRef(({ disabled, className, style, prefix, selectedLabel, suffix, isActive, ...props }: any, ref: any) => {
  return (
    <button
      ref={ref}
      className={classnames('button-dropdown-button flex items-center gap-1.5 text-xs',
        ...
        className)}
      disabled={disabled}
      data-testid={(props as Record<string, any>)['data-testid']}
      style={style}
      role="button"
      {...props}
    >
...
const QueryResultTypeSelector = ({
  ...
  const handleDropdownChange = (open: any) => {
    if (!isActiveTab && open) {
  ...
  const findSelectedLabel = () => {
    if (formatValue != null) {
      const selectedItem = formatOptions.find((item: any) => item.id === formatValue && (item.type === 'item' || !item.type));
  ...
  const enhancedItems = formatOptions.map((item: any) => {
