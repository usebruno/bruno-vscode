...
interface TestResultsLabelProps {
...
const TestResultsLabel = ({
  ...
  const numberOfFailedTests = results.filter((result: any) => result.status === 'fail').length;

  const numberOfAssertions = assertionResults.length;
  const numberOfFailedAssertions = assertionResults.filter((result: any) => result.status === 'fail').length;

  const numberOfPreRequestTests = preRequestTestResults.length;
  const numberOfFailedPreRequestTests = preRequestTestResults.filter((result: any) => result.status === 'fail').length;

  const numberOfPostResponseTests = postResponseTestResults.length;
  const numberOfFailedPostResponseTests = postResponseTestResults.filter((result: any) => result.status === 'fail').length;
