...
interface RequestTabProps {
  ...
  tabIndex?: number;
  collectionRequestTabs?: unknown[];
  folderUid?: string;
  hasOverflow?: boolean;
  setHasOverflow?: boolean;
  dropdownContainerRef?: React.ReactNode;
  menuDropdownRef?: React.ReactNode;
  tabLabelRef?: React.ReactNode;
  dispatch?: boolean;
}
...
const RequestTab = ({
  ...
  const menuDropdownRef = useRef<{ show: () => void } | null>(null);
  ...
  const getMethodColor = (method = '') => {
    const colorMap: Record<string, string> = {
      ...(theme.request.methods as Record<string, string>),
      ...(theme.request as unknown as Record<string, string>)
    };
    return colorMap[method.toLocaleLowerCase()];
  };

  const handleCloseCollectionSettings = (event: any) => {
    if (!collection.draft) {
  ...
  const handleCloseFolderSettings = (event: any) => {
    if (!folder?.draft) {
      return handleCloseClick(event);
    }
  ...
  const handleCloseEnvironmentSettings = (event: any) => {
  ...
  const handleCloseGlobalEnvironmentSettings = (event: any) => {
      ...
      >
        {showConfirmCollectionClose && tab.type === 'collection-settings' && (
          <ConfirmCollectionClose
            ...
            onSaveAndClose={() => {
              (dispatch(saveCollectionRoot(collection.uid)) as unknown as Promise<void>)
                .then(() => {
                  dispatch(closeTabs({
                    tabUids: [tab.uid]
                  }));
        ...
        {showConfirmFolderClose && tab.type === 'folder-settings' && (
          <ConfirmFolderClose
            ...
            onSaveAndClose={() => {
              (dispatch(saveFolderRoot(collection.uid, folder.uid)) as unknown as Promise<void>)
                .then(() => {
                  dispatch(closeTabs({
                    tabUids: [tab.uid]
                  }));
          ...
          <ConfirmCloseEnvironment
            ...
            onSaveAndClose={() => {
              ...
              if (draft?.environmentUid && draft?.variables) {
                (dispatch(saveEnvironment(draft.variables, draft.environmentUid, collection.uid)) as unknown as Promise<void>)
                  .then(() => {
                    dispatch(clearEnvironmentsDraft({ collectionUid: collection.uid }));
                    dispatch(closeTabs({ tabUids: [tab.uid] }));
                    setShowConfirmEnvironmentClose(false);
        ...
        {showConfirmGlobalEnvironmentClose && tab.type === 'global-environment-settings' && (
          <ConfirmCloseEnvironment
            ...
            onSaveAndClose={() => {
              ...
              if (draft?.environmentUid && draft?.variables) {
                (dispatch(saveGlobalEnvironment({ variables: draft.variables, environmentUid: draft.environmentUid })) as unknown as Promise<void>)
                  .then(() => {
                    dispatch(clearGlobalEnvironmentDraft());
                    dispatch(closeTabs({ tabUids: [tab.uid] }));
                    setShowConfirmGlobalEnvironmentClose(false);
    ...
    <StyledWrapper className="flex items-center justify-between tab-container px-2">
      {showConfirmClose && (
        <ConfirmRequestClose
          ...
          onSaveAndClose={() => {
            (dispatch(saveRequest(item.uid, collection.uid)) as unknown as Promise<void>)
              .then(() => {
                dispatch(
                  closeTabs({
                    tabUids: [tab.uid]
...
function RequestTabMenu({
  ...
  async function handleCloseTab(tabUid: any) {
    if (!tabUid) {
      return;
    }
  ...
  async function handleCloseOtherTabs() {
    const otherTabs = collectionRequestTabs.filter((_: any, index: any) => index !== tabIndex);
    await Promise.all(otherTabs.map((tab: any) => handleCloseTab(tab.uid)));
  }

  async function handleCloseTabsToTheLeft() {
    const leftTabs = collectionRequestTabs.filter((_: any, index: any) => index < tabIndex);
    await Promise.all(leftTabs.map((tab: any) => handleCloseTab(tab.uid)));
  }

  async function handleCloseTabsToTheRight() {
    const rightTabs = collectionRequestTabs.filter((_: any, index: any) => index > tabIndex);
    await Promise.all(rightTabs.map((tab: any) => handleCloseTab(tab.uid)));
  }

  function handleCloseSavedTabs() {
    const items = flattenItems(collection?.items);
    const savedTabs = items?.filter?.((item: any) => !hasRequestChanges(item));
    const savedTabIds = savedTabs?.map((item: any) => item.uid) || [];
    dispatch(closeTabs({ tabUids: savedTabIds }));
  }

  async function handleCloseAllTabs() {
    await Promise.all(collectionRequestTabs.map((tab: any) => handleCloseTab(tab.uid)));
  }
