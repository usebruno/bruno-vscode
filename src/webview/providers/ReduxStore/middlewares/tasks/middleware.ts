...
taskMiddleware.startListening({
  ...
  effect: (action, listenerApi) => {
    const state = listenerApi.getState() as { app: { taskQueue: TaskQueueItem[] }; collections: { collections: any[] } };
    const collectionUid = get(action, 'payload.file.meta.collectionUid');

    const openRequestTasks = filter(state.app.taskQueue, { type: taskTypes.OPEN_REQUEST }) as TaskQueueItem[];
...
taskMiddleware.startListening({
  ...
  effect: (action, listenerApi) => {
    ...
    const openExampleTasks = filter(state.app.taskQueue, { type: taskTypes.OPEN_EXAMPLE }) as TaskQueueItem[];
