...
interface PromptVariablesContextValue {
  prompt: (prompts: string[]) => Promise<Record<string, string>>;
}

const PromptVariablesContext = createContext<PromptVariablesContextValue | null>(null);

export function PromptVariablesProvider({
  children
}: any) {
  const [modalState, setModalState] = useState({ open: false, prompts: [], resolve: null, reject: null });

  const prompt = useCallback((prompts: any) => {
  ...
  if (typeof window !== 'undefined') {
    (window as any).promptForVariables = async (prompts: any) => {
      try {
  ...
  return (
    <PromptVariablesContext.Provider value={{ prompt: prompt as (prompts: string[]) => Promise<Record<string, string>> }}>
      {children}
      {modalState.open && (
